...
export default function Select({ navigation }) {
  ...
  const [thereisRoom, setthereisRoom] = useState([]);
  // useEffect(() => {
  ...
  //function initBoard(){
    ...
    setDoc(GamesRef, { 
        ...
        QuestionModal:false,RoomID:Generated_ID,SessionType:SessionType_p, currentPosition:0,CurrentBoard:currentBoard, Host:user.email,Validate:''}//,Correct:false,Wrong:false
      )
...
for (let index = 0; index < readPlayers.length; index++) {
  if(readPlayers[index].Player.length < 3 && (readPlayers[index].SessionType == 'public' && readPlayers[index].GameStatus == 'waiting')){

    navigation.navigate('WaitingRoom', {Room_ID: readPlayers[index].RoomID} ) 
     Update({
       "Players":arrayUnion({Email: user.email,ActiveState: true,Position:0,ID:user.uid,Name:Name})
      },true,'None',readPlayers[index].RoomID)
      break;
    }else{
  let Generated_ID1 = createSession('public');
        navigation.navigate('WaitingRoom', {Room_ID: Generated_ID1} ) 
...
//      // setthereisRoom([...thereisRoom,element.RoomID]); 
//      //  Alert.alert(String(element.RoomID))
